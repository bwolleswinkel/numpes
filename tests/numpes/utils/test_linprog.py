...
from numpes.utils.linprog import Status, OptimizationProgramResult, _solve_lp_scipy, _solve_lp_cvxpy, _solve_lp_pulp
import pytest

from tests.helpers import approx, requires
...
class SolveLPMixin:
    """Mixin for LP backend test cases."""
    ...
    def test_feasible_bounded(self, c: NDArray, A: NDArray, b: NDArray, A_eq: Optional[NDArray], b_eq: Optional[NDArray], bounds: list[tuple[float | None, float | None]], expected_res: OptimizationProgramResult):
        """Test solving a feasible and bounded linear program."""
        res = self.solve_fn(c, A=A, b=b, A_eq=A_eq, b_eq=b_eq, bounds=bounds)
...
class TestSolveLPScipy(SolveLPMixin):
    """Test the `_solve_lp_scipy` function with the SciPy backend."""

    solve_fn = staticmethod(_solve_lp_scipy)


@requires(
    'cvxpy',
    ImportError,
    "The package 'cvxpy' is not installed. Please install it to use the CVXPY backend.",
)
class TestSolveLPCvxpy(SolveLPMixin):
    """Test the `_solve_lp_cvxpy` function with the CVXPY backend."""

    solve_fn = staticmethod(_solve_lp_cvxpy)


@requires(
    'pulp',
    ImportError,
    "The package 'pulp' is not installed. Please install it to use the PuLP backend.",
)
class TestSolveLPPulp(SolveLPMixin):
    """Test the `_solve_lp_pulp` function with the PuLP backend."""

    solve_fn = staticmethod(_solve_lp_pulp)


class TestSolveLP(SolveLPMixin):
    """Test the `pes.utils.solve_lp` function"""
